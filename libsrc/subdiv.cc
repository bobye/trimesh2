			   ...
			   int v0, int v1, int v2, int v3)
{
	...
	float l = 0.375f + 0.25f * std::cos(s2);
...
static inline float loop_update_alpha(int scheme, int n)
{
	if (scheme == SUBDIV_LOOP) {
		...
		if (n == 3)	  return 0.4375f;
			...
			sqr(0.375f + 0.25f * std::cos(M_TWOPIf / n));
	} else { // SUBDIV_LOOP_NEW
		if (n == 3)	  return 0.4375f;
		else if (n == 4)  return 0.5f;
		else if (n == 5)  return 0.545466f;
		else if (n == 6)  return 0.625f;
		float l = 0.375f + 0.25f * std::cos(M_TWOPIf / n);
...
void subdiv(TriMesh *mesh, int scheme /* = SUBDIV_LOOP */)
{
	...
	if (scheme == SUBDIV_LOOP ||
		...
		std::vector<point> oldvertices = mesh->vertices;
