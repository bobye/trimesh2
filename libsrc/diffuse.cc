...
static inline float wt(const point &p1, const point &p2, float invsigma2)
{
	...
	return (d2 >= 9.0f) ? 0.0f : std::exp(-0.5f*d2);
...
// Functor classes for adding scalar, std::vector, or tensor fields on the surface
template <class T>
struct AccumVec {
	const std::vector<T> &field;
	AccumVec(const std::vector<T> &field_) : field(field_)
		{}
...
// a Gaussian of width 1/std::sqrt(invsigma2)
template <class ACCUM, class T>
static void diffuse_vert_field(TriMesh *themesh,
                               std::vector<unsigned> &flags, unsigned &flag_curr,
...
void smooth_mesh(TriMesh *themesh, float sigma)
{
	...
	std::vector<point> dflt(nv), dflt2(nv);
#pragma omp parallel
	{
		// Thread-local flags
		std::vector<unsigned> flags(nv);
		...
		for (int i = 0; i < nf; i++) {
			...
			for (int j = 0; j < 3; j++) {
				...
				dflt[v] += themesh->cornerareas[i][j] /
				  ...
				  std::exp(-0.5f * invsigma2 * len2(d)) * d;
			}
		}
			 ...
			 std::vector<point> &mpoints)
{
	...
	flag_curr++;
	std::vector<int> boundary = themesh->adjacentfaces[v];
	while (!boundary.empty()) {
		int f = boundary.back();
		boundary.pop_back();
...
template void diffuse_vector< float >(TriMesh *, std::vector< float > &, float);
template void diffuse_vector< Vec<2,float> >(TriMesh *, std::vector< Vec<2,float> > &, float);
template void diffuse_vector< Vec<3,float> >(TriMesh *, std::vector< Vec<3,float> > &, float);
template void diffuse_vector< Vec<4,float> >(TriMesh *, std::vector< Vec<4,float> > &, float);
