...
#define eprintf TriMesh::eprintf
  ...
  typedef std::pair<int,int> TriMeshEdge; // (face, edge) pair
  typedef std::pair<float, TriMeshEdge> TriMeshEdgeWithBenefit;
...
static float cosmaxangle(const point &v1, const point &v2, const point &v3)
{
	...
	return 0.5f * std::min(std::min(A,B),C) / (a*b*c); // min cosine == max angle
}
			  ...
			  const point &v3, const point &v4)
{
	...
	return std::max(-cosmaxangle(v1,v2,v3), -cosmaxangle(v1,v3,v4)) -
	       std::max(-cosmaxangle(v1,v2,v4), -cosmaxangle(v2,v3,v4));
}
...
void edgeflip(TriMesh *mesh)
{
	...
	std::priority_queue<TriMeshEdgeWithBenefit> todo;
	for (int i = 0; i < nf; i++) {
		for (int j = 0; j < 3; j++) {
