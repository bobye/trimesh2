...
void normalize_variance(TriMesh *mesh)
{
	...
	float s = 1.0f / std::sqrt(C[0][0] + C[1][1] + C[2][2]);
	scale(mesh, s);
...
void pca_snap(TriMesh *mesh)
{
	...
	if (npos < nv/2)
		first = -first;
	if (std::fabs(first[0]) > std::fabs(first[1])) {
		if (std::fabs(first[0]) > std::fabs(first[2])) {
			first[1] = first[2] = 0;
			first[0] /= std::fabs(first[0]);
		} else {
			first[0] = first[1] = 0;
			first[2] /= std::fabs(first[2]);
		}
	} else {
		if (std::fabs(first[1]) > std::fabs(first[2])) {
			first[0] = first[2] = 0;
			first[1] /= std::fabs(first[1]);
		} else {
			first[0] = first[1] = 0;
			first[2] /= std::fabs(first[2]);
		}
	...
	if (std::fabs(second[0]) > std::fabs(second[1])) {
		if (std::fabs(second[0]) > std::fabs(second[2])) {
			second[1] = second[2] = 0;
			second[0] /= std::fabs(second[0]);
		} else {
			second[0] = second[1] = 0;
			second[2] /= std::fabs(second[2]);
		}
	} else {
		if (std::fabs(second[1]) > std::fabs(second[2])) {
			second[0] = second[2] = 0;
			second[1] /= std::fabs(second[1]);
		} else {
			second[0] = second[1] = 0;
			second[2] /= std::fabs(second[2]);
		}
