...
void GLCamera::startspin()
{
	...
	if (dt < SPIN_TIME && std::fabs(spinspeed) > SPIN_SPEED)
		dospin = true;
}
...
void GLCamera::rotate(int mousex, int mousey, xform &xf)
{
	...
	float spinamount = std::sqrt(sqr(nx-ox)+sqr(ny-oy));
...
void GLCamera::movez(int mousex, int mousey, xform &xf)
{
	...
	float dz = click_depth * (std::exp(-delta) - 1.0f);
...
void GLCamera::relight(int mousex, int mousey)
{
	...
	float theta = (float)MAX_LIGHT * std::min(std::sqrt(x*x+y*y), 1.0f);
	float phi = std::atan2(y, x);

	lightdir[0] = std::sin(theta)*std::cos(phi);
	lightdir[1] = std::sin(theta)*std::sin(phi);
	lightdir[2] = std::cos(theta);
}
			   ...
			   const point &scene_center, float scene_size)
{
	...
	pixscale = 2.0f / float(std::max(P[0]*V[2], P[5]*V[3]));
	...
	if (scene_center[2] < 0 && len(scene_center) > scene_size) {
		...
		xmin = std::min(std::max((GLint) (cx - csize), V[0]), V[0]+V[2]);
		xmax = std::min(std::max((GLint) (cx + csize), V[0]), V[0]+V[2]);
		ymin = std::min(std::max((GLint) (cy - csize), V[1]), V[1]+V[3]);
		ymax = std::min(std::max((GLint) (cy + csize), V[1]), V[1]+V[3]);
	}
	...
	float f = (float)csize / std::max(V[2], V[3]);
	f = std::min(std::max(2.0f * f - 1.0f, 0.0f), 1.0f);
	...
	tb_screen_size = std::min(tb_screen_size, 0.7f * std::min(V[2], V[3]));
	tb_screen_size = std::max(tb_screen_size, 0.3f * std::min(V[2], V[3]));
}
...
void GLCamera::setupGL(const point &scene_center, float scene_size) const
{
	...
	float fardist  = std::max(-(scene_center[2] - scene_size),
			     scene_size / DOF);
	float neardist = std::max(-(scene_center[2] + scene_size),
			     scene_size / MAXDOF);
	...
	float diag = std::sqrt(float(sqr(width) + sqr(height)));
