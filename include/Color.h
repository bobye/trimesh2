...
private:
	...
	static inline float inv_srgb_nonlinearity(float x)
	{
		...
		if (x > (0.0031308f * 12.92f))
			return std::pow((x + 0.055f) * (1.0f / 1.055f), 2.4f);
		else
...
public:
	...
	const Color gamma(float g) const
	{
		...
		float g1 = 1.0f / g;
		return Color(std::pow(v[0],g1), std::pow(v[1],g1), std::pow(v[2],g1));
	}
	...
	const Color ungamma(float g) const
	{
		...
		return Color(std::pow(v[0],g), std::pow(v[1],g), std::pow(v[2],g));
	}
