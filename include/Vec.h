...
Supports the following operations:
	...
	v1 = std::min(v2, v3);	// Componentwise min/max
	v1 = std::sin(v2);		// Componentwise - all the usual functions...
...
namespace std {
 ...
 template <int D, class T> 
 static inline trimesh::Vec<D,T> min(const trimesh::Vec<D,T> &v, const T &w, const T &x)
 { 
	trimesh::Vec<D,T> result(VEC_UNINITIALIZED); 
	for (int i = 0; i < D; i++) 
		result[i] = min(v[i], w, x); 
	return result; 
 } 
 template <int D, class T> 
 static inline trimesh::Vec<D,T> min(const trimesh::Vec<D,T> &v, const trimesh::Vec<D,T> &w, const trimesh::Vec<D,T> &x) 
 { 
        trimesh::Vec<D,T> result(VEC_UNINITIALIZED); 
	for (int i = 0; i < D; i++) 
		result[i] = min(v[i], w[i], x[i]); 
	return result; 
 }

 template <int D, class T> 
 static inline trimesh::Vec<D,T> max(const trimesh::Vec<D,T> &v, const T &w, const T &x)
 { 
	trimesh::Vec<D,T> result(VEC_UNINITIALIZED); 
	for (int i = 0; i < D; i++) 
		result[i] = max(v[i], w, x); 
	return result; 
 } 
 template <int D, class T> 
 static inline trimesh::Vec<D,T> max(const trimesh::Vec<D,T> &v, const trimesh::Vec<D,T> &w, const trimesh::Vec<D,T> &x) 
 { 
        trimesh::Vec<D,T> result(VEC_UNINITIALIZED); 
	for (int i = 0; i < D; i++) 
		result[i] = max(v[i], w[i], x[i]); 
	return result; 
 }
